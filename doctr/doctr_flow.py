    ...
    try:
        ...
        if language in sym_spell_dicts:
            ...
            suggestions = sym_spell.lookup(
                text, symspellpy.Verbosity.CLOSEST, max_edit_distance=2
            )
...
def visualize_intermediate_results(image_path, detected_text, ax_image_synthesized):
    ...
    ax_image_synthesized.imshow(
        detected_text.synthesize(font_family="DejaVuSans.ttf")[0]
    )
...
def process_image(image_path: Path, output_folder: Path, reader, model, processor):
    ...
    image_with_boxes = visualize_intermediate_results(
        image_path, result, ax_image_synthesized
    )

    image_with_boxes.savefig(
        output_folder / f"{image_path.parts[-1]}_detected_boxes.png"
    )
    fig_image_synthesized.savefig(
        output_folder / f"{image_path.parts[-1]}_synthesized_image.png", dpi=900
    )
